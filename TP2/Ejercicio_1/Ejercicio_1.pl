...
verificar(safety_valve_components_having_effects) :-  
    ...
    estado( safety_valve_components_having_effects, no ), writeln('Coordination is required in order to render and color the equipment.');   
...
verificar(piloto) :- 
    estado(piloto, ok), writeln('Set the safety valve according to the instauctions');
    estado(piloto, no), writeln('Pilot full service and reinstallation');
    (
        estado(piloto, desconocido), 
        (
        (estado(leakage_prevention_between_sit_and_orifice, ok), writeln('Verificar estado de Piloto'));
...
verificar(control_valve_sensors_blocked) :- 
                estado(control_valve_sensors_blocked, ok), writeln('Cleaning and troubleshooting of the sensing pipes') ; 
...
verificar(valve_status_closed) :- 
                estado(valve_status_closed, ok), writeln('Place the safety valve in open position') ; 
...
verificar(relief_valve_ok_with_10_percent_more_pressure) :- 
                ...
                estado(relief_valve_ok_with_10_percent_more_pressure, ok), writeln('Safety function is appropriate') ; 
...
verificar(preventable_lackage_between_sit_and_orifice) :- 
                estado( preventable_lackage_between_sit_and_orifice, ok ), writeln('Set the safety valve according to the instructions ');
                estado( preventable_lackage_between_sit_and_orifice, no ), writeln('Replace Sit and Orifice and put the safety valve into circuit');
                (
